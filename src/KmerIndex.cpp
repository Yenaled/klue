...
std::set<int> computeComplement(const std::set<int>& target, const std::map<int, std::set<int>>& k_map) {
    std::set<int> universalSet;
    for (const auto& kv : k_map) {
        universalSet.insert(kv.second.begin(), kv.second.end());
    }
    std::set<int> complement;
    std::set_difference(universalSet.begin(), universalSet.end(), target.begin(), target.end(), std::inserter(complement, complement.begin()));

    return complement;
}
// END for set operation
        ...
        workers.emplace_back(
          [&, u_i] {
            ...
            for (auto unitig_x : unitigs_v[u_i]) { // Go through unitigs in the batch labeled u_i
              ...
              std::string sequence_to_prepend, sequence_to_append; // If we need to prepend/append sequences to contigs
